...
bool CERNStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CERNStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CERNStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CERNStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CERNStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CERNStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CERNStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CERNStake::GetUniqueness() const
{
    //The unique identifier for a ERN stake is the outpoint
...
CBlockIndex* CERNStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CERNStake::ContextCheck(int nHeight, uint32_t nTime)
{
