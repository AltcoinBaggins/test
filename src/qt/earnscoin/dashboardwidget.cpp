...
#include "qt/earnscoin/dashboardwidget.h"
#include "qt/earnscoin/forms/ui_dashboardwidget.h"
#include "qt/earnscoin/sendconfirmdialog.h"
#include "qt/earnscoin/txrow.h"
...
DashboardWidget::DashboardWidget(earnscoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareERN, "square-chart-earnscoin");
    setCssProperty(ui->labelERN, "text-chart-earnscoin");
    ...
    setCssProperty(ui->labelAmountERN, "text-stake-earnscoin-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &earnscoinGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &earnscoinGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &earnscoinGUI::openFAQ);
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            earnscoin = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalERN += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesERN.append(earnscoin);

        int max = std::max(earnscoin, earnscoin);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(133,194,38));
    ...
    set0->append(chartData->valuesERN);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalERN > 0) {
        setCssProperty(ui->labelAmountERN, "text-stake-earnscoin");
    } else {
        setCssProperty(ui->labelAmountERN, "text-stake-earnscoin-disable");
    }
    forceUpdateStyle({ui->labelAmountERN});
    ui->labelAmountERN->setText(GUIUtil::formatBalance(chartData->totalERN, nDisplayUnit));
