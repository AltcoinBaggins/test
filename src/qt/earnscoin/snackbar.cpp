...
#include "qt/earnscoin/forms/ui_snackbar.h"
#include "qt/earnscoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(earnscoinGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &earnscoinGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
