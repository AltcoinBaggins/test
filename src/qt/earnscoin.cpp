...
#include "qt/earnscoin/splash.h"
#include "qt/earnscoin/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("earnscoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in earnscoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in earnscoin.qrc)
...
/** Class encapsulating earnscoin startup and shutdown.
...
/** Main earnscoin application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (earnscoinGUI)
...
{
    window = new earnscoinGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &earnscoinGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &earnscoinGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(earnscoinGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(earnscoinGUI::DEFAULT_WALLET);
        ...
        // earnscoin: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &earnscoinGUI::handlePaymentRequest);
        connect(window, &earnscoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. Earnscoin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(earnscoin_locale);
    Q_INIT_RESOURCE(earnscoin);
    ...
    /// 6. Determine availability of data directory and parse earnscoin.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("Earnscoin"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("Earnscoin"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // earnscoin: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("Earnscoin didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
