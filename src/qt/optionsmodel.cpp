...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::ERN);
    ...
    if (settings.contains("nAnonymizeearnscoinAmount") || reset)
        SoftSetArg("-anonymizeearnscoinamount", settings.value("nAnonymizeearnscoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in earnscoin.cpp during shotdown to also remove the window positions
