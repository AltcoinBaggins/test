...
#else
    const char* pszModule = "earnscoin";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\earnscoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\earnscoin
// Mac: ~/Library/Application Support/earnscoin
// Unix: ~/.earnscoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ERN";
#else
    fs::path pathRet;
...
{
    fs::path pathConfigFile(GetArg("-conf", "earnscoin.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty earnscoin.conf if it does not exist
        FILE* configFile = fsbridge::fopen(GetConfigFile(), "a");
        if (configFile != NULL)
        {
            std::string strHeader = "addnode=108.61.220.138:6662\n"
                                    "addnode=207.246.102.131:6662\n"
                                    "addnode=45.77.68.89:6662\n"
                                    "addnode=45.76.168.47:6662\n";
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override earnscoin.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", "earnscoind.pid"));
