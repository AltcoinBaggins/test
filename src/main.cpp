...
#if defined(NDEBUG)
#error "earnscoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uearnscoin) are considered zero fee (for relaying, mining and transaction creation)
...
bool RecalculateERNSupply(int nHeightStart)
{
	AssertLockHeld(cs_main);
	...
	uiInterface.ShowProgress(_("Recalculating ERN supply..."), 0);
	while (true) {
		if (pindex->nHeight % 1000 == 0) {
			LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
			int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
			uiInterface.ShowProgress(_("Recalculating ERN supply..."), percent);
		}
...
{
    CAmount nMnPayment = 0;

    if (nHeight <= 1000) { 
        nMnPayment = 0;
    } else if (nHeight <= 20000) { 
        nMnPayment = 0.33 * COIN;
    } else if (nHeight <= 30000) { 
        nMnPayment = 0.28 * COIN;
    } else if (nHeight <= 40000) { 
        nMnPayment = 0.23 * COIN;
    } else if (nHeight <= 50000) { 
        nMnPayment = 0.18 * COIN;
    } else if (nHeight <= 60000) { 
        nMnPayment = 0.13 * COIN;
    } else if (nHeight <= 80000) { 
        nMnPayment = 0.78 * COIN;
    } else if (nHeight <= 100000) { 
...
{
    util::ThreadRename("earnscoin-scriptch");
    ...
    if (pindex->nHeight > last_pow_block) {
      ...
      if (nDevReward > 0) {
        ...
        for (const auto& output : block.vtx[1].vout) {
            // enable 2 satoshi threshold to avoid mistakes with rounding
            if (output.scriptPubKey == DEV_SCRIPT && output.nValue > (nDevReward - 2) && output.nValue < (nDevReward + 2)) {
                DevPaid = true;
                break;
            }
        }
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> earnscoinInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                earnscoinInputs.push_back(stakeIn);
        }
        const bool hasERNInputs = !earnscoinInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasERNInputs) {
                    ...
                    for (const CTxIn& earnscoinIn : earnscoinInputs)
                        if(earnscoinIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasERNInputs) {
                            for (const CTxIn& stakeIn : earnscoinInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // earnscoin: We use certain sporks during IBD, so check to see if they are
