...
*Setup instructions for a gitian build of earnscoin using a VM or physical system.*

Gitian is the deterministic build process that is used to build the earnscoin
...
to the earnscoin GitHub release page.
...
- [Building earnscoin](#building-earnscoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/earnscoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/earnscoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/earnscoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/earnscoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/earnscoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/earnscoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/earnscoinvps-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@earnscoin.trade) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/earnscoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/earnscoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/earnscoin-osx-*-build.assert
```
